...
def generic_mapper_test(data, expected_message, expected_number_of_elements, element=None):
    """
    Generic test function for mapping records different formats and schemas to a normalized Python object based on
    the Crossref schema.

    Args:
        data (str): The data to be mapped.
        expected_message (str): The expected message from the mapper.
        expected_number_of_elements (int): The expected number of top level elements after mapping.
        element (str, optional): An optional element to check within the article.
    """
